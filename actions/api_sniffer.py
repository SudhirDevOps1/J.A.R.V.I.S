...
def sniff_web_api(
    ...
    # Extract clean Python code block from Gemini output if present
    code_match = re.search(r"```(?:python)?\s*([\s\S]*?)```", analysis)
    if code_match:
        script_code = code_match.group(1).strip()
    else:
        # If no explicit code block was returned, generate a clean runnable template with comments
        safe_analysis_comment = "\n".join(f"# {line}" for line in analysis.splitlines())
        script_code = (
            f"'''\nReverse Engineering Notes for {url}:\n'''\n"
            f"{safe_analysis_comment}\n\n"
            "import requests\n\n"
            f"URL = '{url}'\n"
    ...
    try:
        ...
        print(f"[APISniffer] Save script note: {e}")
